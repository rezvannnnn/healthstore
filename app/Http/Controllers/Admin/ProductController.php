...
{
    public function __construct(protected CartService $cartService, protected InventoryService $inventoryService) {}
    ...
    {
        return Inertia::render('Admin/Products/Create', [...$this->formOptions(), 'product' => null]);
    }
    ...
    public function edit(Product $product): Response
    {
        ...
        return Inertia::render('Admin/Products/Edit', [
            ...
            'product' => [
                ...
                'barcode' => $product->barcode, 'brand_id' => $product->brand_id, 'category_id' => $product->category_id,
                'product_type' => $product->product_type, 'unit' => $product->unit, 'quantity_per_unit' => $product->quantity_per_unit,
                'short_description' => $product->short_description, 'description' => $product->description,
                'expiry_date' => $product->expiry_date?->format('Y-m-d'), 'main_image' => $product->main_image,
    ...
    {
        return ['brands' => Brand::query()->where('is_active', true)->orderBy('name')->get(['id', 'name']), 'categories' => Category::query()->where('is_active', true)->orderBy('name')->get(['id', 'name'])];
    }
    ...
    protected function validatedData(Request $request, ?Product $product = null): array
    {
        return $request->validate([
            ...
            'product_type' => ['nullable', 'string', 'max:255'], 'unit' => ['nullable', 'string', 'max:255'],
            'quantity_per_unit' => ['nullable', 'integer', 'min:1'], 'short_description' => ['nullable', 'string', 'max:1000'],
            'description' => ['nullable', 'string'], 'expiry_date' => ['nullable', 'date', 'after_or_equal:today'],
            'main_image' => ['nullable', 'string', 'max:2048'], 'is_active' => ['boolean'], 'is_featured' => ['boolean'],
            'sort_order' => ['nullable', 'integer', 'min:0'], 'price' => ['required', 'numeric', 'min:0'],
    ...
    protected function productData(array $data): array
    {
        ...
        return [
            ...
            'product_type' => $data['product_type'] ?? null, 'unit' => $data['unit'] ?? null, 'quantity_per_unit' => $data['quantity_per_unit'] ?? null,
            'short_description' => $data['short_description'] ?? null, 'description' => $data['description'] ?? null,
            'seo_title' => $name, 'seo_description' => $description !== '' ? Str::limit($description, 160, '') : null,
            'canonical_url' => url('/products/'.($slug !== '' ? $slug : Str::slug($name))),
            'expiry_date' => $data['expiry_date'] ?? null, 'main_image' => $data['main_image'] ?? null,
            'is_active' => (bool) ($data['is_active'] ?? false), 'is_featured' => (bool) ($data['is_featured'] ?? false),
    ...
    protected function syncRetailPrice(Product $product, array $data): void
    {
        ProductPrice::updateOrCreate(
            ['product_id' => $product->id, 'price_type' => 'retail', 'min_quantity' => 1],
            ['price' => $data['price'], 'compare_at_price' => $data['compare_at_price'] ?? null, 'is_active' => true, 'starts_at' => null, 'ends_at' => null],
        );
    }
}
