...
class OrderController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $orders = Order::query()->where('user_id', $user->id)->select([
            'id', 'user_id', 'order_number', 'status', 'payment_status', 'subtotal',
            'discount_amount', 'shipping_amount', 'total_amount', 'currency', 'created_at',
    ...
    public function cancel(Request $request, string $orderNumber): RedirectResponse
    {
        ...
        $order = Order::query()->where('order_number', $orderNumber)->where('user_id', $user->id)->firstOrFail();
