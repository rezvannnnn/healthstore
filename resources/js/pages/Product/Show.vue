...
<template>
    <div dir="rtl" class="min-h-screen bg-dh-surface text-dh-ink">
        ...
        <main class="mx-auto max-w-7xl px-4 py-6 pb-28 sm:px-6 sm:py-8 lg:px-8 lg:pb-12">
            <nav class="mb-6 flex flex-wrap items-center gap-2 text-sm text-dh-muted" aria-label="مسیر صفحه">
                <Link href="/" class="transition hover:text-dh-700">خانه</Link>
                <span>/</span>
                <Link href="/products" class="transition hover:text-dh-700">محصولات</Link>
                <template v-if="product.category_slug">
                    <span>/</span>
                    <Link :href="'/categories/' + product.category_slug" class="transition hover:text-dh-700">
                        {{ product.category }}
                    </Link>
                </template>
                <span>/</span>
                <span class="line-clamp-1 font-medium text-dh-800">{{ product.name }}</span>
            </nav>

            <section class="overflow-hidden rounded-[2rem] border border-dh-100 bg-white shadow-[0_16px_50px_rgba(20,108,114,0.07)]">
                <div class="grid lg:grid-cols-[1.02fr_0.98fr]">
                    <div class="border-b border-dh-100 p-4 sm:p-6 lg:border-b-0 lg:border-l lg:p-8">
                        <div class="relative aspect-square overflow-hidden rounded-[1.6rem] bg-gradient-to-br from-dh-50 via-white to-dh-green-50">
                            <span
                                v-if="product.compare_at_price && product.price !== null && product.compare_at_price > product.price"
                                class="absolute right-4 top-4 z-10 rounded-full bg-dh-green-600 px-3 py-1.5 text-xs font-bold text-white shadow-sm"
                            >
                                پیشنهاد ویژه
                            </span>
                            ...
                            />
                            <div v-else class="flex h-full flex-col items-center justify-center gap-3 text-dh-muted">
                                <span class="flex h-16 w-16 items-center justify-center rounded-2xl bg-white shadow-sm">
                                    <svg viewBox="0 0 24 24" class="h-8 w-8" fill="none" stroke="currentColor" stroke-width="1.5">
                                        <rect x="3" y="4" width="18" height="16" rx="2" />
                                        <path d="M7 15l3-3 3 3 2-2 3 3" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </span>
                                بدون تصویر
                            </div>
                        ...
                        <div v-if="gallery.length > 1" class="mt-4 grid grid-cols-5 gap-2.5 sm:gap-3">
                            <button
                                v-for="(image, index) in gallery"
                                :key="image"
                                type="button"
                                class="aspect-square overflow-hidden rounded-2xl border bg-white p-1.5 transition"
                                :class="selectedImage === image ? 'border-dh-600 ring-2 ring-dh-100' : 'border-dh-100 hover:border-dh-300'"
                    ...
                    <div class="flex flex-col p-5 sm:p-7 lg:p-10">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span v-if="product.product_type" class="rounded-full bg-dh-green-50 px-3 py-1.5 text-xs font-semibold text-dh-green-700">
                                {{ product.product_type }}
                            </span>
                        </div>

                        <h1 class="mt-5 text-2xl leading-[1.35] font-extrabold text-dh-900 sm:text-3xl lg:text-[2.15rem]">
                            {{ product.name }}
                        </h1>

                        <p v-if="product.short_description" class="mt-4 text-sm leading-7 text-dh-muted sm:text-base">
                        ...
                        <div class="flex items-end justify-between gap-4">
                            <div>
                                <div class="text-xs font-medium text-dh-muted">قیمت مصرف‌کننده</div>
                                <div class="mt-1 text-2xl font-extrabold text-dh-800 sm:text-3xl">
                            ...
                            >
                                <span class="h-2 w-2 rounded-full bg-dh-green-500"></span>
                                موجود در انبار
                            </div>
                            <div
                                v-else
                        ...
                        <div v-if="product.available && product.price !== null" class="mt-6 rounded-2xl bg-dh-surface p-3 sm:p-4">
                        ...
                        <div class="mt-6 grid gap-2.5 sm:grid-cols-3">
                            <div class="rounded-2xl border border-dh-100 bg-white p-3">
                                <svg viewBox="0 0 24 24" class="h-5 w-5 text-dh-700" fill="none" stroke="currentColor" stroke-width="1.6">
                                    <path d="M12 3l7 3v5c0 4.5-3 8.2-7 10-4-1.8-7-5.5-7-10V6l7-3Z" />
                                    <path d="M9 12l2 2 4-4" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                                <div class="mt-2 text-xs font-bold text-dh-800">خرید مطمئن</div>
                                <div class="mt-1 text-[11px] leading-5 text-dh-muted">اطلاعات شفاف محصول</div>
                            </div>
                            <div class="rounded-2xl border border-dh-100 bg-white p-3">
                                <svg viewBox="0 0 24 24" class="h-5 w-5 text-dh-green-600" fill="none" stroke="currentColor" stroke-width="1.6">
                                    <path d="M3 7h11v10H3zM14 10h4l3 3v4h-7z" stroke-linejoin="round" />
                                    <circle cx="7" cy="18" r="1.5" /><circle cx="18" cy="18" r="1.5" />
                                </svg>
                                <div class="mt-2 text-xs font-bold text-dh-800">ارسال سفارش</div>
                                <div class="mt-1 text-[11px] leading-5 text-dh-muted">پیگیری ساده و شفاف</div>
                            </div>
                            <div class="rounded-2xl border border-dh-100 bg-white p-3">
                                <svg viewBox="0 0 24 24" class="h-5 w-5 text-dh-700" fill="none" stroke="currentColor" stroke-width="1.6">
                                ...
                                <div class="mt-2 text-xs font-bold text-dh-800">پشتیبانی</div>
                                <div class="mt-1 text-[11px] leading-5 text-dh-muted">همراه شما در خرید</div>
                            </div>
                        </div>

                        <div v-if="product.description" class="mt-7 border-t border-dh-100 pt-6">
                            <h2 class="text-lg font-extrabold text-dh-900">درباره محصول</h2>
                            <p class="mt-3 whitespace-pre-line text-sm leading-8 text-dh-muted">
                                {{ product.description }}
                            </p>
                        </div>
                    </div>
            ...
            >
                <div class="flex items-end justify-between gap-4">
                    <div>
                        <span class="text-xs font-bold text-dh-green-600">جزئیات محصول</span>
                        <h2 class="mt-1 text-2xl font-extrabold text-dh-900">مشخصات فنی</h2>
                    </div>
                </div>

                <dl class="mt-6 overflow-hidden rounded-2xl border border-dh-100">
                    <div
                    ...
                    >
                        <dt class="text-sm font-bold text-dh-muted">{{ key }}</dt>
                        <dd class="break-words text-sm font-medium text-dh-800">
            ...
            <section
                v-if="product.sku || product.barcode || product.unit || product.quantity_per_unit || product.product_type"
                class="mt-6 rounded-[2rem] border border-dh-100 bg-white p-5 shadow-[0_12px_40px_rgba(20,108,114,0.05)] sm:p-7 lg:p-8"
            >
                <h2 class="text-xl font-extrabold text-dh-900">اطلاعات محصول</h2>
                <div class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <div v-if="product.sku" class="rounded-2xl bg-dh-surface p-4">
                        <div class="text-xs text-dh-muted">کد کالا</div>
                        <div class="mt-1 font-bold text-dh-800">{{ product.sku }}</div>
                    </div>
                    <div v-if="product.barcode" class="rounded-2xl bg-dh-surface p-4">
                        <div class="text-xs text-dh-muted">بارکد</div>
                        <div class="mt-1 font-bold text-dh-800">{{ product.barcode }}</div>
                    </div>
                    <div v-if="product.unit" class="rounded-2xl bg-dh-surface p-4">
                        <div class="text-xs text-dh-muted">واحد</div>
                        <div class="mt-1 font-bold text-dh-800">{{ product.unit }}</div>
                    </div>
                    <div v-if="product.quantity_per_unit" class="rounded-2xl bg-dh-surface p-4">
                        <div class="text-xs text-dh-muted">تعداد در واحد</div>
                        <div class="mt-1 font-bold text-dh-800">{{ product.quantity_per_unit }}</div>
                    </div>
                </div>
            </section>
            ...
            >
                <div class="flex items-end justify-between gap-4">
                    <div>
                        <span class="text-xs font-bold text-dh-green-600">پیشنهاد برای شما</span>
                        <h2 class="mt-1 text-2xl font-extrabold text-dh-900">محصولات مرتبط</h2>
                    </div>
                    <Link
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    >
                        <div class="aspect-square overflow-hidden rounded-xl bg-dh-surface">
                            <img
                                v-if="related.image"
                                :src="related.image"
                                :alt="related.name"
                            ...
                            />
                            <div v-else class="flex h-full items-center justify-center text-xs text-dh-muted">بدون تصویر</div>
                        </div>
                        <h3 class="mt-3 line-clamp-2 text-sm leading-6 font-bold text-dh-800 transition group-hover:text-dh-700">
                            {{ related.name }}
                        </h3>
                        <p class="mt-2 text-sm font-extrabold text-dh-700">{{ formatPrice(related.price) }}</p>
                    </Link>
                </div>
            </section>
        </main>

        <nav class="fixed inset-x-0 bottom-0 z-50 border-t border-dh-100 bg-white/95 px-3 py-2 shadow-[0_-8px_30px_rgba(20,108,114,0.08)] backdrop-blur sm:hidden">
            <div class="mx-auto grid max-w-md grid-cols-3 gap-2">
                <Link href="/" class="flex flex-col items-center gap-1 rounded-xl py-1.5 text-[11px] font-semibold text-dh-muted">
                    <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8">
                        <path d="M3 10.5 12 3l9 7.5v9a1 1 0 0 1-1 1h-5v-6h-6v6H4a1 1 0 0 1-1-1v-9Z" stroke-linejoin="round" />
                    </svg>
                    خانه
                </Link>
                <Link href="/products" class="flex flex-col items-center gap-1 rounded-xl bg-dh-50 py-1.5 text-[11px] font-bold text-dh-700">
                    <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8">
                        <path d="M4 6h16M4 12h16M4 18h16" stroke-linecap="round" />
                    </svg>
                    محصولات
                </Link>
                <Link href="/cart" class="flex flex-col items-center gap-1 rounded-xl py-1.5 text-[11px] font-semibold text-dh-muted">
                    <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8">
                        <path d="M4 5h2l1.5 10.2a2 2 0 0 0 2 1.8h7.6a2 2 0 0 0 1.9-1.5L21 8H7" stroke-linecap="round" stroke-linejoin="round" />
