...
const { customer, orders } = defineProps<{
    customer: Customer;
    orders: Order[];
}>();
...
<template>
    <Head :title="`مشتری ${customer.name || customer.phone || customer.id}`" />
        ...
        <div class="mx-auto max-w-5xl">
            <div class="mb-8 flex items-center justify-between gap-4">
                <div>
                    ...
                    <h1 class="text-2xl font-bold">{{ customer.name || 'بدون نام' }}</h1>
                </div>
                <Link
            ...
            <section class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                ...
                <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                        ...
                        <p class="mt-1 font-medium">{{ customer.name || '—' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 font-medium">{{ customer.phone_verified ? 'تأیید شده' : 'تأیید نشده' }}</p>
                    </div>
                </div>
            </section>
                    ...
                    <table class="min-w-full text-right text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="order in orders" :key="order.id">
                                ...
                                <td class="px-4 py-4">{{ order.items_count }}</td>
                                <td class="px-4 py-4 font-medium">{{ formatAmount(order.total_amount) }} ریال</td>
                                <td class="px-4 py-4">{{ statusLabel(order.status) }}</td>
                                <td class="px-4 py-4">{{ paymentLabel(order.payment_status) }}</td>
                                <td class="px-4 py-4 text-xs text-gray-500">
                                    {{ order.created_at ? new Date(order.created_at).toLocaleDateString('fa-IR') : '—' }}
                                </td>
                                <td class="px-4 py-4">
                                    <Link :href="`/admin/orders/${order.id}`" class="font-medium text-blue-700 hover:underline">مشاهده</Link>
                                </td>
                            </tr>
                            <tr v-if="orders.length === 0">
                                <td colspan="7" class="px-4 py-12 text-center text-gray-500">سفارشی برای این مشتری ثبت نشده است.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
