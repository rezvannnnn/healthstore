...
<template>
    ...
    <div class="mx-auto max-w-7xl space-y-6 p-6" dir="rtl">
        ...
        <form
            class="grid gap-4 rounded-xl border bg-white p-5 shadow-sm md:grid-cols-3"
        ...
        <section class="overflow-hidden rounded-xl border bg-white shadow-sm">
            <div class="border-b p-5">
                <h2 class="text-lg font-semibold">۱۰ محصول پرفروش</h2>
            </div>
            <div v-if="props.top_products.length" class="overflow-x-auto">
                <table class="w-full text-right text-sm">
                    ...
                    <tbody>
                        <tr
                            v-for="product in props.top_products"
                            :key="product.product_id"
                            class="border-t"
                        >
                            <td class="p-3">{{ product.name }}</td>
                            <td class="p-3">{{ money(product.quantity) }}</td>
                            <td class="p-3">{{ money(product.sales) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
