...
const form = useForm({
    product_id: 0,
    warehouse_id: props.warehouses[0]?.id ?? 0,
    quantity: 0,
    minimum_quantity: 0,
    batch_number: '',
    expiry_date: '',
...
const createInventory = () =>
    form.post('/admin/inventory', {
        onSuccess: () => {
            ...
            form.reset();
            form.warehouse_id = props.warehouses[0]?.id ?? 0;
        },
    });
const adjust = (inventoryId: number, delta: number) => {
    adjustment.quantity_delta = delta;
    adjustment.post(`/admin/inventory/${inventoryId}/adjust`, {
        preserveScroll: true,
                    ...
                    <table class="min-w-full text-right text-sm">
                        <thead class="border-b bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3">رزرو</th>
                                <th class="px-4 py-3">قابل فروش</th>
                                <th class="px-4 py-3">حداقل</th>
                            ...
                            <tr v-for="product in products" :key="product.id">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex flex-wrap gap-2">
                                        <template v-if="product.inventory_id"
                                            ><button
                                                @click="
                                                    adjust(
                                            ...
                                            >
                                                −۱</button
                                            ><Link
                                                :href="`/admin/inventory/${product.inventory_id}/movements`"
                                                class="rounded border px-3 py-1"
                                                >گردش</Link
                                            ></template
                                        ><button
