            ...
            <section class="grid gap-6 lg:grid-cols-[1fr_350px]">
                <div class="space-y-5">
                    ...
                    <section v-if="hasChanges" class="rounded-3xl border border-amber-200 bg-amber-50 p-5 shadow-sm md:p-6">
                        ...
                        <div v-if="hasPriceChanges" class="mt-5 space-y-2"><h3 class="text-sm font-black text-amber-900">تغییر قیمت</h3><div v-for="change in priceChanges" :key="`price-${change.product_id}`" class="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white/70 p-3 text-sm"><strong>{{ change.product_name }}</strong><span><span class="text-dh-muted line-through">{{ formatPrice(change.old_price) }}</span><span class="mx-2">←</span><strong>{{ formatPrice(change.new_price ?? 0) }}</strong></span></div></div>
                        <div v-if="hasOutOfStockItems" class="mt-5 space-y-2"><h3 class="text-sm font-black text-amber-900">محصولات ناموجود</h3><div v-for="change in availabilityChanges" :key="`stock-${change.product_id}`" class="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white/70 p-3 text-sm"><strong>{{ change.product_name }}</strong><span class="font-bold text-red-600">ناموجود · <span class="font-normal text-dh-muted line-through">{{ formatPrice(change.old_price) }}</span></span></div></div>
                    </section>
