    ...
    <div dir="rtl" class="min-h-screen bg-dh-surface pb-24 text-dh-ink lg:pb-10">
        ...
        <main class="mx-auto max-w-7xl space-y-8 px-4 py-6 md:px-6 md:py-10">
            ...
            <section class="rounded-[2rem] bg-dh-800 p-7 text-white shadow-xl shadow-dh-900/10 md:p-10">
                ...
                <h1 class="mt-3 text-3xl font-black md:text-5xl">محصولات را بر اساس دسته پیدا کنید.</h1>
                <p class="mt-4 max-w-2xl text-sm leading-7 text-dh-50/85 md:text-base">
                    از مراقبت پوست و مو تا ویتامین‌ها و محصولات بهداشتی، دسته مورد نظر را انتخاب کنید.
                </p>
            ...
            <section v-if="categories.length" class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                >
                    <div class="aspect-[16/10] overflow-hidden bg-dh-50">
                        ...
                        />
                        <div v-else class="flex h-full items-center justify-center p-6 text-center text-xl font-black text-dh-700">
                            {{ category.name }}
                        </div>
                    </div>
                    <div class="p-5">
                        <span v-if="category.parent" class="text-xs font-semibold text-dh-muted">
                            {{ category.parent.name }}
                        </span>
                        <h2 class="mt-1 text-xl font-black text-dh-800 group-hover:text-dh-700">{{ category.name }}</h2>
                        <p v-if="category.description" class="mt-2 line-clamp-2 text-sm leading-7 text-dh-muted">
                            {{ category.description }}
                        </p>
                        <div class="mt-5 flex items-center justify-between border-t border-dh-100 pt-4">
                            <span class="text-xs text-dh-muted">{{ category.products_count.toLocaleString('fa-IR') }} محصول فعال</span>
                            <span class="text-sm font-black text-dh-700">مشاهده ←</span>
                        </div>
                        <div v-if="category.children.length" class="mt-4 flex flex-wrap gap-2">
                            <span
        ...
        <nav class="fixed inset-x-0 bottom-0 z-50 border-t border-dh-100 bg-white/95 px-3 py-2 backdrop-blur lg:hidden">
            <div class="mx-auto grid max-w-md grid-cols-3 gap-2 text-center text-[11px] font-bold text-dh-muted">
                ...
                <Link href="/categories" class="rounded-xl bg-dh-50 px-2 py-2 text-dh-700">دسته‌ها</Link>
            </div>
        </nav>
    </div>
</template>
