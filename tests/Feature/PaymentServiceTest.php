...
class PaymentServiceTest extends TestCase
{
    ...
    public function test_payment_can_be_created_for_an_order(): void
    {
        ...
        $this->assertEquals(360000, (float) $payment->amount);
    ...
    {
        $user = User::factory()->create();
        $order = $this->createOrder($user, 123456);
        $service = new PaymentService(new InventoryReservationService);
        $payment = $service->create($order);
    ...
    public function test_payment_cannot_be_created_for_already_paid_order(): void
    {
        $user = User::factory()->create();
        $order = $this->createOrder($user, 360000);
        $order->update([
    ...
    public function test_successful_payment_marks_payment_as_paid_order_as_paid_and_consumes_reservations(): void
    {
        ...
        $product = Product::create([
            'brand_id' => null, 'category_id' => null, 'name' => 'Payment Product',
            'slug' => 'payment-product-'.uniqid(), 'sku' => 'PAY-'.uniqid(),
        ...
        $warehouse = Warehouse::create([
            'name' => 'Payment Warehouse', 'code' => 'PAY-WH-'.uniqid(),
        ...
        $this->assertEquals('TX-123456', $payment->transaction_id);
        ...
        $this->assertDatabaseHas('payments', [
            'id' => $payment->id, 'status' => 'paid', 'transaction_id' => 'TX-123456',
        ]);
    }
    ...
    {
        $user = User::factory()->create();
        $order = $this->createOrder($user, 200000);
        $product = Product::create([
            'brand_id' => null, 'category_id' => null, 'name' => 'Failed Payment Product',
            'slug' => 'failed-payment-product-'.uniqid(), 'sku' => 'FAIL-'.uniqid(),
            'product_type' => 'physical', 'unit' => 'piece', 'quantity_per_unit' => 1,
            'short_description' => null, 'description' => null, 'specifications' => null,
            'expiry_date' => null, 'main_image' => null, 'is_active' => true,
            'is_featured' => false, 'sort_order' => 1,
        ...
        $warehouse = Warehouse::create([
            'name' => 'Failed Payment Warehouse', 'code' => 'FAIL-WH-'.uniqid(),
            'description' => null, 'is_active' => true,
        ]);
        $inventory = Inventory::create([
            'product_id' => $product->id, 'warehouse_id' => $warehouse->id, 'quantity' => 7,
            'minimum_quantity' => 1, 'batch_number' => 'FAIL-BATCH-'.uniqid(),
            'expiry_date' => null, 'is_active' => true,
        ]);
        $reservation = InventoryReservation::create([
            'order_id' => $order->id, 'product_id' => $product->id, 'inventory_id' => $inventory->id,
        ...
        $inventory->decrement('quantity', 3);
        $inventory->refresh();
    ...
    {
        $user = User::factory()->create();
        $order = $this->createOrder($user, 150000);
        $product = Product::create([
            'brand_id' => null, 'category_id' => null, 'name' => 'Cancelled Payment Product',
            'slug' => 'cancelled-payment-product-'.uniqid(), 'sku' => 'CANCEL-'.uniqid(),
            'product_type' => 'physical', 'unit' => 'piece', 'quantity_per_unit' => 1,
            'short_description' => null, 'description' => null, 'specifications' => null,
            'expiry_date' => null, 'main_image' => null, 'is_active' => true,
            'is_featured' => false, 'sort_order' => 1,
        ...
        $warehouse = Warehouse::create([
            'name' => 'Cancelled Payment Warehouse', 'code' => 'CANCEL-WH-'.uniqid(),
            'description' => null, 'is_active' => true,
        ]);
        $inventory = Inventory::create([
            'product_id' => $product->id, 'warehouse_id' => $warehouse->id, 'quantity' => 5,
            'minimum_quantity' => 1, 'batch_number' => 'CANCEL-BATCH-'.uniqid(),
            'expiry_date' => null, 'is_active' => true,
        ]);
        $reservation = InventoryReservation::create([
            'order_id' => $order->id, 'product_id' => $product->id, 'inventory_id' => $inventory->id,
    ...
    public function test_paid_payment_cannot_be_marked_as_failed_or_paid_again(): void
    {
        ...
        $this->assertTrue($paymentService->markAsPaid($payment, 'TX-999999'));
        $payment->refresh();
        $this->assertFalse($paymentService->markAsFailed($payment, 'Should not change'));
        $payment->refresh();
        $this->assertFalse($paymentService->markAsPaid($payment, 'TX-000000'));
        $payment->refresh();
        $this->assertEquals('paid', $payment->status);
